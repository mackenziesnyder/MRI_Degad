...
class WATUNet(nn.Module):
    ...
    def _make_enc_block(self, in_c, out_c):
        return nn.Sequential(
            nn.Conv2d(in_c, out_c, 3, padding=1), nn.BatchNorm2d(out_c), nn.LeakyReLU(inplace=True),
    ...
    def forward(self, x):
        ...
        # Bridge
        bridge_cbam = self.cbam4(self.wat_layer3(self.bridge(self.pool(enc3_cbam)), wat3))

        # Decoder
        up1 = F.interpolate(bridge_cbam, scale_factor=2, mode='bilinear', align_corners=True)
        dec1_cbam = self.cbam_dec1(self.dec1(torch.cat([up1, enc3_cbam], dim=1)))
        
        up2 = F.interpolate(dec1_cbam, scale_factor=2, mode='bilinear', align_corners=True)
        dec2_cbam = self.cbam_dec2(self.dec2(torch.cat([up2, enc2_cbam], dim=1)))

        up3 = F.interpolate(dec2_cbam, scale_factor=2, mode='bilinear', align_corners=True)
