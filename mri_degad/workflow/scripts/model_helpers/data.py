...
def _pad_and_crop_volume(vol):
    ...
    vol = torch.nn.functional.pad(
        vol, (0, pad_needed[2][1], 0, pad_needed[1][1], 0, pad_needed[0][1])
    )
...
class NiftiTestDataset(Dataset):
    ...
    def __init__(self, view, image_path, enable_sap=True):
        ...
        motion_vol = (motion_vol - motion_vol.min()) / (
            motion_vol.max() - motion_vol.min() + 1e-8
        )  # EXACT same normalization
        self.motion_padded = _pad_and_crop_volume(
            motion_vol
        )  # EXACT same padding/cropping
